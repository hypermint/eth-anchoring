...
library RLP {
    ...
    function distance(RLPItem memory from, RLPItem memory to) internal pure returns (uint256) {
        int256 d = int256(to._unsafe_memPtr - from._unsafe_memPtr);
        if (d < 0) {
            d = -d;
        }
        return uint256(d);
    ...
    /// @dev Decode an RLPItem into a bytes20. This will not work if the
    /// RLPItem is a list.
    /// @param self The RLPItem.
    /// @return The decoded string.
    function toBytes20(RLPItem memory self) internal pure returns (bytes20 data, bool valid) {
        (uint256 num, bool val) = toUint(self, 20);
        if (!val) {
            return (data, valid);
        }
        return (bytes20(uint160(num)), val);
    }
    ...
    function _copyToBytes(uint256 btsPtr, bytes memory tgt, uint256 btsLen) private pure {
